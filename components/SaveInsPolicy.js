...
const data = [
  ...
  { sr: 5, loanAcNo: 'RMC-24O19D4-005326', customerName: 'AAKASH', regnNo: 'DL1LAK2440', vehDelDt: '21-04-2023', vehMake: 'Bajaj Maxima Cng', chasisNo: '00404' },
  { sr: 6, loanAcNo: 'RMC-24D188C-005443', customerName: 'AAKASH', regnNo: 'DL1LAK9475', vehDelDt: '05-12-2024', vehMake: 'Bajaj Maxima Cng', chasisNo: '22973' },
  { sr: 7, loanAcNo: 'RMC-22O1953-003792', customerName: 'AAMIR', regnNo: 'DL1LAK0408', vehDelDt: '03-10-2022', vehMake: 'Continental Engines Ltd', chasisNo: '41715' },
  { sr: 8, loanAcNo: 'RMC-25F1D2E-005651', customerName: 'AAMIR ALI', regnNo: 'HR55AN7857', vehDelDt: '22-01-2025', vehMake: 'Continental Engines Ltd', chasisNo: '43103' },
  { sr: 9, loanAcNo: 'RMC-25M1B9B-005757', customerName: 'AAMIR CHOUDHARY', regnNo: 'DL1LAP1571', vehDelDt: '28-02-2025', vehMake: 'Bajaj Maxima Cng', chasisNo: '01632' },
  { sr: 10, loanAcNo: 'RMC-24D1E1E-005444', customerName: 'AAMIR CHOUDHARY', regnNo: 'DL1LAK9383', vehDelDt: '27-11-2024', vehMake: 'Bajaj Maxima Cng', chasisNo: '22543' },
];

const PageNumbers = () => {
  const totalRecords = 4318;
...
export default function SaveInsPolicy() {
  ...
  const handleRowClick = (loan) => {
    if (selectedLoan && selectedLoan.sr === loan.sr) {
      setSelectedLoan(null); // Deselect if the same row is clicked again
    } else {
      // In a real app, you might fetch more details here.
      // For this example, we'll merge static data for the form.
      setSelectedLoan({
        ...loan,
        extAcNo: 'DLGUGUSV06188',
        vehChNo: 'MD2B25BX6RWH22543',
        vehEngNo: 'AZXWRH58326',
        vehModel: 'Nov 2024',
      });
    }
  };
        ...
        <table className="min-w-full bg-white">
          ...
          <tbody className="text-gray-700 text-sm font-light">
            {data.map((row) => (
              <tr 
                key={row.sr} 
                className={`border-b border-gray-200 hover:bg-blue-100 cursor-pointer ${selectedLoan?.sr === row.sr ? 'bg-green-200' : ''}`}
                onClick={() => handleRowClick(row)}
              >
                <td className="py-3 px-6 text-left whitespace-nowrap">{row.sr}</td>
                <td className="py-3 px-6 text-left">
                  <span className="font-medium text-blue-600">{row.loanAcNo}</span>
                </td>
        ...
        <div className="mt-10 p-8 bg-blue-50 rounded-2xl shadow-lg border border-blue-200 animate-fade-in-up">
          <div className="grid grid-cols-2 gap-x-16 gap-y-4 text-base">
            ...
            <div className="space-y-3">
              <div className="flex">
                <label className="w-40 font-semibold text-blue-900">Internal Ac No.:</label>
                <span className="font-medium text-gray-800">{selectedLoan.loanAcNo}</span>
              </div>
              <div className="flex">
                <label className="w-40 font-semibold text-blue-900">Ext Ac No.:</label>
                <span className="font-medium text-gray-800">{selectedLoan.extAcNo}</span>
              </div>
              <div className="flex">
                <label className="w-40 font-semibold text-blue-900">Customer Name:</label>
                <span className="font-medium text-gray-800">{selectedLoan.customerName}</span>
              </div>
              <div className="flex">
                <label className="w-40 font-semibold text-blue-900">Veh Ch. No.:</label>
                <span className="font-medium text-gray-800">{selectedLoan.vehChNo}</span>
              </div>
              <div className="flex">
                <label className="w-40 font-semibold text-blue-900">Veh Eng No.:</label>
                <span className="font-medium text-gray-800">{selectedLoan.vehEngNo}</span>
              </div>
            </div>
            {/* Right Column */}
            <div className="space-y-3">
              <div className="flex">
                <label className="w-48 font-semibold text-blue-900">Veh Make:</label>
                <span className="font-medium text-gray-800">{selectedLoan.vehMake}</span>
              </div>
              <div className="flex">
                <label className="w-48 font-semibold text-blue-900">Veh Model:</label>
                <span className="font-medium text-gray-800">{selectedLoan.vehModel}</span>
              </div>
              <div className="flex">
                <label className="w-48 font-semibold text-blue-900">Veh Regn. No.:</label>
                <span className="font-medium text-gray-800">{selectedLoan.regnNo}</span>
              </div>
            </div>
          </div>
          <div className="flex justify-center items-center gap-8 mt-8">
              <label htmlFor="file-upload" className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-3 rounded-lg shadow-xl border-2 border-blue-700 transition-all duration-200">
