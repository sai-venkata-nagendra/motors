...
export default function EditGuarantor() {
  ...
  return (
    <div className="animate-fade-in-up max-w-6xl mx-auto bg-gradient-to-br from-blue-100 via-white to-teal-100 rounded-3xl shadow-2xl p-10 mt-10 border border-blue-200 relative backdrop-blur-xl bg-opacity-80">
      <h2 className="text-4xl font-extrabold mb-10 text-blue-900 border-b-4 border-teal-200 pb-4 tracking-wider drop-shadow-lg text-center uppercase">Edit Guarantor</h2>
      <form className="space-y-10">
        <div className="grid grid-cols-12 gap-6 items-end">
          ...
          <div className="col-span-10 grid grid-cols-10 gap-4 items-end">
            <div className="col-span-2">
              ...
              <input type="text" value={form.guarantorId} onChange={handleInput} name="guarantorId" className="w-full border border-blue-200 rounded px-3 py-2 bg-blue-50 text-blue-900 font-bold" />
            </div>
            <div className="col-span-1">
              <label className="block text-blue-900 font-semibold mb-1">Title</label>
              <select name="guarantorTitle" value={form.guarantorTitle} onChange={handleInput} className="w-full border border-blue-200 rounded px-2 py-2">
                <option>Mr.</option>
                <option>Ms.</option>
                <option>Mrs.</option>
              </select>
            ...
            <div className="col-span-3">
              <label className="block text-blue-900 font-semibold mb-1">Guarantor Name</label>
              <input name="guarantorName" value={form.guarantorName} onChange={handleInput} placeholder="Guarantor Name" className="w-full border border-blue-200 rounded px-3 py-2" />
            </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          <input placeholder="Guarantor Mobile 1" value={form.guarantorMobile1} onChange={e => setForm({ ...form, guarantorMobile1: e.target.value })} className="border border-blue-200 rounded px-3 py-2" />
          <input placeholder="Guarantor Mobile 2" value={form.guarantorMobile2} onChange={e => setForm({ ...form, guarantorMobile2: e.target.value })} className="border border-blue-200 rounded px-3 py-2" />
          <input placeholder="Guarantor Email" value={form.guarantorEmail} onChange={e => setForm({ ...form, guarantorEmail: e.target.value })} className="border border-blue-200 rounded px-3 py-2" />
        </div>
        <div className="grid grid-cols-2 gap-6">
          <input placeholder="Guarantor Veh Regn." value={form.vehRegn} onChange={e => setForm({ ...form, vehRegn: e.target.value })} className="border border-blue-200 rounded px-3 py-2" />
        </div>
